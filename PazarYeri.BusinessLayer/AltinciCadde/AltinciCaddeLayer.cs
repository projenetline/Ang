...
namespace PazarYeri.BusinessLayer.AltinciCadde
{
    public class AltinciCaddeLayer
    {
        private readonly DatabaseLayer _databaseLayer;
        private readonly net_LogoTransferSettings _transferSettings;

        private const string _entegrationName = "AltinciCadde";

        public AltinciCaddeLayer()
        {
            _databaseLayer = new DatabaseLayer();
            _transferSettings = _databaseLayer.GetLogoTransferSettings(_entegrationName);
        }

        public bool TransferOrders(net_AltinciCadde netorder, out string result)
        {
            ...
            try
            {
                var OrderRef = _databaseLayer.GetOrderRef(_transferSettings.OrderTransferSpeCode, netorder.OrderNumber, "");
                ...
                if (OrderRef == 0)
                {
                    ...
                    //int shipinforef = CreateSevkiyatAdresi(_settings.OrderTransferArpCode, netorder.Phone, netorder.ShipInfoName, netorder.Addr);
                    //if (shipinforef > 0)
                    //{
                    ...
                    transactions_lines[transactions_lines.Count - 1].FieldByName("MASTER_CODE").Value = netorder.ProductCode;
                    transactions_lines[transactions_lines.Count - 1].FieldByName("AUXIL_CODE").Value = _transferSettings.OrderTransferSpeCode;
                ...
                else
                {
                    ...
                    transactions_lines.AppendLine();
                    transactions_lines[transactions_lines.Count - 1].FieldByName("TYPE").Value = 0;
                    transactions_lines[transactions_lines.Count - 1].FieldByName("MASTER_CODE").Value = netorder.ProductName;
                    transactions_lines[transactions_lines.Count - 1].FieldByName("AUXIL_CODE").Value = _transferSettings.OrderTransferSpeCode;
                    transactions_lines[transactions_lines.Count - 1].FieldByName("QUANTITY").Value = netorder.Quantity;
                    transactions_lines[transactions_lines.Count - 1].FieldByName("PRICE").Value = Math.Round((netorder.Price / 1.18), 2);
                    transactions_lines[transactions_lines.Count - 1].FieldByName("VAT_RATE").Value = 18;
